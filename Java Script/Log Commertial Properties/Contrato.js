...
function Onload_Contract() {
    if ((Xrm.Page.ui.getFormType() == 2) && (Xrm.Page.getAttribute('new_produtoinserido').getValue() == false) && (Xrm.Page.getAttribute("new_tipodecontrato").getValue() == 1)) {
...
function VerificaCDU() {
    ...
    DebbugerConsole(parseFloat(resto.toFixed(2)));
    DebbugerConsole(parseFloat(valorTotal.toFixed(2)));
    DebbugerConsole(parseFloat(valorCDUContrato.toFixed(2)));
    DebbugerConsole(parseFloat(resultado.toFixed(2)));
...
function CopiaParaAditivo() {
    if (Xrm.Page.ui.getFormType() == 2) {
        if (Xrm.Page.getAttribute("new_tipodecontrato").getValue() == 2) { //Aditivo Contratual
            CopiaFiadoresDoContrato();
        } else if (Xrm.Page.getAttribute("new_tipodecontrato").getValue() == 3) { //CDU
            CopiaCduDoContrato();
        }
    }
}

function CopiaCduDoContrato() {
    if ((Xrm.Page.getAttribute("new_cdu").getValue() == 1) &&
        (Xrm.Page.getAttribute("originatingcontract").getValue() !== null) &&
        (Xrm.Page.getAttribute("new_cduinserido").getValue() == false) &&
        (Xrm.Page.getAttribute('new_statusdocontrato').getValue() == 1)) {
        var entityRetrive = getEntityNodesDouble("new_cdu", "new_contratoid", Xrm.Page.getAttribute("originatingcontract").getValue()[0].id, "statecode", "Active");
        if (entityRetrive.length != 0) {
            for (x = 0; x < entityRetrive.length; x++) {
                var entityNodeRetriveCDU = entityRetrive[x];
                var id = entityNodeRetriveCDU.selectSingleNode("q1:new_cduid");
                if (id != null) { id = id.text; } else { id = ""; }
                var valor = entityNodeRetriveCDU.selectSingleNode("q1:new_valor");
                if (valor != null) { valor = valor.text; } else { valor = 0; }
                var data = entityNodeRetriveCDU.selectSingleNode("q1:new_datadevencimento");
                if (data != null) { data = data.text; } else { data = ""; }
                /*Insere CDU*/
                var xml = "<?xml version='1.0' encoding='utf-8'?>" +
                    "<soap:Envelope xmlns:soap='http://schemas.xmlsoap.org/soap/envelope/'" +
                    " xmlns:xsi='http://www.w3.org/2001/XMLSchema-instance'" +
                    ...
                    "<soap:Body>" +
                    "<Create xmlns='http://schemas.microsoft.com/crm/2007/WebServices'>" +
                    "<entity xsi:type='new_cdu'>" +
                        "<new_datadevencimento>" + data + "</new_datadevencimento>" +
                        "<new_valor>" + valor + "</new_valor>" +
                        "<new_contratoid>" + Xrm.Page.data.entity.getId() + "</new_contratoid>" +
                    "</entity>" +
                    "</Create>" +
                    "</soap:Body>" +
                    "</soap:Envelope>";
        ...
        Xrm.Page.getAttribute("new_cduinserido").setValue(true);
        Xrm.Page.data.entity.save("save");
    }
}

function CopiaFiadoresDoContrato() {
    if ((Xrm.Page.getAttribute("new_tipodegarantia").getValue() == 1) &&
            (Xrm.Page.getAttribute("originatingcontract").getValue() !== null) &&
            (Xrm.Page.getAttribute("new_fiadoresinseridos").getValue() == false) &&
            (Xrm.Page.getAttribute('new_statusdocontrato').getValue() == 1)) {
        var entityRetrive = getEntityNodesDouble("new_fiador", "new_contratoid", Xrm.Page.getAttribute("originatingcontract").getValue()[0].id, "statecode", "Active");
        if (entityRetrive.length != 0) {
            for (x = 0; x < entityRetrive.length; x++) {
                var entityNodeRetrive = entityRetrive[x];
                var idFiador = entityNodeRetrive.selectSingleNode("q1:new_pessoaid");
                if (idFiador != null) { idFiador = idFiador.text; } else { idFiador = ""; }
                var nomeFiador = entityNodeRetrive.selectSingleNode("q1:new_name");
                if (nomeFiador != null) { nomeFiador = nomeFiador.text; } else { nomeFiador = ""; }
                /*Insere Fiadores*/
                var xml = "<?xml version='1.0' encoding='utf-8'?>" +
                    "<soap:Envelope xmlns:soap='http://schemas.xmlsoap.org/soap/envelope/'" +
                    " xmlns:xsi='http://www.w3.org/2001/XMLSchema-instance'" +
                    ...
                    "<soap:Body>" +
                    "<Create xmlns='http://schemas.microsoft.com/crm/2007/WebServices'>" +
                    "<entity xsi:type='new_fiador'>" +
                        "<new_pessoaid>" + idFiador + "</new_pessoaid>" +
                        "<new_contratoid>" + Xrm.Page.data.entity.getId() + "</new_contratoid>" +
                        "<new_name>" + nomeFiador + "</new_name>" +
                    "</entity>" +
                    "</Create>" +
                    "</soap:Body>" +
                    "</soap:Envelope>";
        ...
        Xrm.Page.getAttribute("new_fiadoresinseridos").setValue(true);
    }
    CopiaDescontosDoContrato();
}

function CopiaDescontosDoContrato() {
    if ((Xrm.Page.getAttribute("new_desconto").getValue() == 1) &&
                (Xrm.Page.getAttribute("originatingcontract").getValue() !== null) &&
                (Xrm.Page.getAttribute("new_descontosinseridos").getValue() == false) &&
                (Xrm.Page.getAttribute('new_statusdocontrato').getValue() == 1)) {
        var entityRetrive = getEntityNodesDouble("new_descontocontrato", "new_contratoid", Xrm.Page.getAttribute("originatingcontract").getValue()[0].id, "statecode", "Active");
        if (entityRetrive.length != 0) {
            for (x = 0; x < entityRetrive.length; x++) {
                var entityNodeRetrive = entityRetrive[x];
                var codigo = entityNodeRetrive.selectSingleNode("q1:new_codigo");
                if (codigo != null) { codigo = codigo.text; } else { codigo = ""; }
                var descontoPercentual = entityNodeRetrive.selectSingleNode("q1:new_desconto");
                if (descontoPercentual != null) { descontoPercentual = descontoPercentual.text; } else { descontoPercentual = 0; }
                var descontoValor = entityNodeRetrive.selectSingleNode("q1:new_descontovalor");
                if (descontoValor != null) { descontoValor = descontoValor.text; } else { descontoValor = 0; }
                var nome = entityNodeRetrive.selectSingleNode("q1:new_name");
                if (nome != null) { nome = nome.text; } else { nome = ""; }
                var quantidade = entityNodeRetrive.selectSingleNode("q1:new_quantidademeses");
                if (quantidade != null) { quantidade = quantidade.text; } else { quantidade = 0; }
                var desconto = 0;
                var nomeCampo = "";
                if (descontoPercentual > 0) {
                    desconto = descontoPercentual;
                    nomeCampo = "new_desconto";
                } else {
                    desconto = descontoValor;
                    nomeCampo = "new_descontovalor";
                }
                /*Insere Descontos*/
                var xml = "<?xml version='1.0' encoding='utf-8'?>" +
                    "<soap:Envelope xmlns:soap='http://schemas.xmlsoap.org/soap/envelope/'" +
                    ...
                    "<soap:Body>" +
                    "<Create xmlns='http://schemas.microsoft.com/crm/2007/WebServices'>" +
                    "<entity xsi:type='new_descontocontrato'>" +
                        "<new_codigo>" + codigo + "</new_codigo>" +
                        "<new_contratoid>" + Xrm.Page.data.entity.getId() + "</new_contratoid>" +
                        "<new_name>" + nome + "</new_name>" +
                        "<new_quantidademeses>" + quantidade + "</new_quantidademeses>" +
                        "<" + nomeCampo + ">" + (desconto * 1) + "</" + nomeCampo + ">" +
                    "</entity>" +
                    "</Create>" +
                    "</soap:Body>" +
                    "</soap:Envelope>";
        ...
        Xrm.Page.getAttribute("new_descontosinseridos").setValue(true);
    }
    CopiaAluguelEscalonadoDoContrato();
}

function CopiaAluguelEscalonadoDoContrato() {
    if ((Xrm.Page.getAttribute("new_aluguelescalonado").getValue() == 1) &&
                   (Xrm.Page.getAttribute("originatingcontract").getValue() !== null) &&
                   (Xrm.Page.getAttribute("new_alugueisescalonadosinseridos").getValue() == false) &&
                   (Xrm.Page.getAttribute('new_statusdocontrato').getValue() == 1)) {
        var entityRetrive = getEntityNodesDouble("new_aluguelescalonado", "new_contratoid", Xrm.Page.getAttribute("originatingcontract").getValue()[0].id, "statecode", "Active");
        if (entityRetrive.length != 0) {
            for (x = 0; x < entityRetrive.length; x++) {
                var entityNodeRetrive = entityRetrive[x];
                var codigo = entityNodeRetrive.selectSingleNode("q1:new_name");
                if (codigo != null) { codigo = codigo.text; } else { codigo = 0; }
                var quantidade = entityNodeRetrive.selectSingleNode("q1:new_quantidade");
                if (quantidade != null) { quantidade = quantidade.text; } else { quantidade = 0; }
                var contador = entityNodeRetrive.selectSingleNode("q1:new_contador");
                if (contador != null) { contador = contador.text; } else { contador = 0; }
                var valor = entityNodeRetrive.selectSingleNode("q1:new_valor");
                if (valor != null) { valor = valor.text; } else { valor = 0; }
                /*Insere Escalonados*/
                var xml = "<?xml version='1.0' encoding='utf-8'?>" +
                    "<soap:Envelope xmlns:soap='http://schemas.xmlsoap.org/soap/envelope/'" +
                    ...
                    "<soap:Body>" +
                    "<Create xmlns='http://schemas.microsoft.com/crm/2007/WebServices'>" +
                    "<entity xsi:type='new_aluguelescalonado'>" +
                        "<new_name>" + codigo + "</new_name>" +
                        "<new_contratoid>" + Xrm.Page.data.entity.getId() + "</new_contratoid>" +
                        "<new_quantidade>" + quantidade + "</new_quantidade>" +
                        "<new_contador>" + contador + "</new_contador>" +
                        "<new_valor>" + valor + "</new_valor>" +
                    "</entity>" +
                    "</Create>" +
                    "</soap:Body>" +
        ...
        Xrm.Page.getAttribute("new_alugueisescalonadosinseridos").setValue(true);
    }
    CopiaReceitaVariavelDoContrato();
}

function CopiaReceitaVariavelDoContrato() {
    if ((Xrm.Page.getAttribute("new_receitavariavel").getValue() !== null) &&
                     (Xrm.Page.getAttribute("originatingcontract").getValue() !== null) &&
                     (Xrm.Page.getAttribute("new_receitasvariveisinseridas").getValue() == false) &&
                     (Xrm.Page.getAttribute('new_statusdocontrato').getValue() == 1)) {
        var entityRetrive = getEntityNodesDouble("new_receitavariavel", "new_contratoid", Xrm.Page.getAttribute("originatingcontract").getValue()[0].id, "statecode", "Active");
        if (entityRetrive.length != 0) {
            for (x = 0; x < entityRetrive.length; x++) {
                var entityNodeRetrive = entityRetrive[x];
                var name = entityNodeRetrive.selectSingleNode("q1:new_name");
                if (name != null) { name = name.text; } else { name = ""; }
                var cliente = entityNodeRetrive.selectSingleNode("q1:new_clienteid");
                if (cliente != null) { cliente = cliente.text; } else { cliente = ""; }
                var dataApuracao = entityNodeRetrive.selectSingleNode("q1:new_datadaapuracao");
                if (dataApuracao != null) { dataApuracao = dataApuracao.text; } else { dataApuracao = ""; }
                var mes = entityNodeRetrive.selectSingleNode("q1:new_mes");
                if (mes != null) { mes = mes.text; } else { mes = ""; }
                var ano = entityNodeRetrive.selectSingleNode("q1:new_ano");
                if (ano != null) { ano = ano.text; } else { ano = ""; }
                var aluguelBase = entityNodeRetrive.selectSingleNode("q1:new_aluguelbase");
                if (aluguelBase != null) { aluguelBase = aluguelBase.text; } else { aluguelBase = 0; }
                var renda = entityNodeRetrive.selectSingleNode("q1:new_renda");
                if (renda != null) { renda = renda.text; } else { renda = 0; }
                var arrecadacao = entityNodeRetrive.selectSingleNode("q1:new_arrecadacao");
                if (arrecadacao != null) { arrecadacao = arrecadacao.text; } else { arrecadacao = 0; }
                var diferenca = entityNodeRetrive.selectSingleNode("q1:new_diferenca");
                if (diferenca != null) { diferenca = diferenca.text; } else { diferenca = 0; }
                var aluguelCobrado = entityNodeRetrive.selectSingleNode("q1:new_aluguelcobrado");
                if (aluguelCobrado != null) { aluguelCobrado = aluguelCobrado.text; } else { aluguelCobrado = 0; }
                /*Insere Variavel*/
                var xml = "<?xml version='1.0' encoding='utf-8'?>" +
                    "<soap:Envelope xmlns:soap='http://schemas.xmlsoap.org/soap/envelope/'" +
                    " xmlns:xsi='http://www.w3.org/2001/XMLSchema-instance'" +
                    ...
                    "<soap:Body>" +
                    "<Create xmlns='http://schemas.microsoft.com/crm/2007/WebServices'>" +
                    "<entity xsi:type='new_receitavariavel'>" +
                       "<new_name>" + name + "</new_name>" +
                       "<new_clienteid>" + cliente + "</new_clienteid>" +
                       "<new_datadaapuracao>" + dataApuracao + "</new_datadaapuracao>" +
                       "<new_mes>" + mes + "</new_mes>" +
                       "<new_ano>" + ano + "</new_ano>" +
                       "<new_aluguelbase>" + aluguelBase + "</new_aluguelbase>" +
                       "<new_renda>" + renda + "</new_renda>" +
                       "<new_arrecadacao>" + arrecadacao + "</new_arrecadacao>" +
                       "<new_diferenca>" + diferenca + "</new_diferenca>" +
                       "<new_aluguelcobrado>" + aluguelCobrado + "</new_aluguelcobrado>" +
                       "<new_contratoid>" + Xrm.Page.data.entity.getId() + "</new_contratoid>" +
                    "</entity>" +
                    "</Create>" +
                    "</soap:Body>" +
        ...
        Xrm.Page.getAttribute("new_receitasvariveisinseridas").setValue(true);
    }
    CopiaItensDoContrato();
}

function CopiaItensDoContrato() {
    if ((Xrm.Page.getAttribute("originatingcontract").getValue() !== null) &&
        (Xrm.Page.getAttribute("new_produtoinserido").getValue() == false) &&
        (Xrm.Page.getAttribute('new_statusdocontrato').getValue() == 1)) {
        var entityRetrive = getEntityNodes("contractdetail", "contractid", Xrm.Page.getAttribute("originatingcontract").getValue()[0].id);
        if (entityRetrive.length != 0) {
            for (x = 0; x < entityRetrive.length; x++) {
                var entityNodeRetrive = entityRetrive[x];
                var empreendimento = entityNodeRetrive.selectSingleNode("q1:new_emprendimentoid");
                if (empreendimento != null) { empreendimento = empreendimento.text; } else { empreendimento = ""; }
                var productId = entityNodeRetrive.selectSingleNode("q1:productid");
                if (productId != null) { productId = productId.text; } else { productId = ""; }
                var quantidade = entityNodeRetrive.selectSingleNode("q1:new_quantidade");
                if (quantidade != null) { quantidade = quantidade.text; } else { quantidade = 0; }
                var uomId = entityNodeRetrive.selectSingleNode("q1:uomid");
                if (uomId != null) { uomId = uomId.text; } else { uomId = ""; }
                var valorUnitario = entityNodeRetrive.selectSingleNode("q1:new_valornominal");
                if (valorUnitario != null) { valorUnitario = valorUnitario.text; } else { valorUnitario = 0; }
                var valroBruto = entityNodeRetrive.selectSingleNode("q1:price");
                if (valroBruto != null) { valroBruto = valroBruto.text; } else { valroBruto = 0; }
                var name = entityNodeRetrive.selectSingleNode("q1:title");
                if (name != null) { name = name.text; } else { name = ""; }
                var dataInicio = Xrm.Page.getAttribute('activeon').getValue();
                dataInicio = (dataInicio.getMonth() + 1) + '/' + dataInicio.getDate() + '/' + dataInicio.getFullYear();
                var dataFim = Xrm.Page.getAttribute('expireson').getValue();
                dataFim = (dataFim.getMonth() + 1) + '/' + dataFim.getDate() + '/' + dataFim.getFullYear();
                var atividade = entityNodeRetrive.selectSingleNode("q1:new_atividade");
                if (atividade != null) { atividade = atividade.text; } else { atividade = ""; }
                var condicoesEspeciais = entityNodeRetrive.selectSingleNode("q1:new_condicoesespeciais");
                if (condicoesEspeciais != null) { condicoesEspeciais = condicoesEspeciais.text; } else { condicoesEspeciais = ""; }
                var valorMedio = entityNodeRetrive.selectSingleNode("q1:new_valormedio");
                if (valorMedio != null) { valorMedio = valorMedio.text; } else { valorMedio = 0; }
                var valor = entityNodeRetrive.selectSingleNode("q1:new_valor");
                if (valor != null) { valor = valor.text; } else { valor = 0; }
                var acrescimos = entityNodeRetrive.selectSingleNode("q1:new_acrescimos");
                if (acrescimos != null) { acrescimos = acrescimos.text; } else { acrescimos = 0; }
                var condominio = entityNodeRetrive.selectSingleNode("q1:new_condominio");
                if (condominio != null) { condominio = condominio.text; } else { condominio = 0; }
                var iptu = entityNodeRetrive.selectSingleNode("q1:new_iptu");
                if (iptu != null) { iptu = iptu.text; } else { iptu = 0; }
                var energiaAgua = entityNodeRetrive.selectSingleNode("q1:new_energiaagua");
                if (energiaAgua != null) { energiaAgua = energiaAgua.text; } else { energiaAgua = 0; }
                var totalDeDespesas = entityNodeRetrive.selectSingleNode("q1:new_totaldedespesas");
                if (totalDeDespesas != null) { totalDeDespesas = totalDeDespesas.text; } else { totalDeDespesas = 0; }
                var total = entityNodeRetrive.selectSingleNode("q1:new_total");
                if (total != null) { total = total.text; } else { total = 0; }
                var discount = entityNodeRetrive.selectSingleNode("q1:discount");
                if (discount != null) { discount = discount.text; } else { discount = 0; }
                var cliente = entityNodeRetrive.selectSingleNode("q1:customerid");
                if (cliente != null) { cliente = cliente.text; } else { cliente = ""; }
                var statusLinha = entityNodeRetrive.selectSingleNode("q1:new_statusdalinha");
                if (statusLinha != null) { statusLinha = statusLinha.text; if (statusLinha == 2) { statusLinha = 1; } } else { statusLinha = 1; }
                /*Insere Itens*/
                var xml = "<?xml version='1.0' encoding='utf-8'?>" +
                "<soap:Envelope xmlns:soap='http://schemas.xmlsoap.org/soap/envelope/'" +
                " xmlns:xsi='http://www.w3.org/2001/XMLSchema-instance'" +
                ...
                "<Create xmlns='http://schemas.microsoft.com/crm/2007/WebServices'>" +
                "<entity xsi:type='contractdetail'>" +
                    "<contractid>" + Xrm.Page.data.entity.getId() + "</contractid>" +
                    "<new_emprendimentoid>" + empreendimento + "</new_emprendimentoid>" +
                    "<productid>" + productId + "</productid>" +
                    "<new_quantidade>" + quantidade + "</new_quantidade>" +
                    "<uomid>" + uomId + "</uomid>" +
                    "<new_valornominal>" + valorUnitario + "</new_valornominal>" +
                    ...
                    "<activeon>" + dataInicio + "</activeon>" +
                    "<expireson>" + dataFim + "</expireson>" +
                    "<new_statusdalinha>" + statusLinha + "</new_statusdalinha>" +
                    "<new_atividade>" + atividade + "</new_atividade>" +
                    "<new_condicoesespeciais>" + condicoesEspeciais + "</new_condicoesespeciais>" +
                    "<new_valormedio>" + valorMedio + "</new_valormedio>" +
                    "<new_valor>" + valor + "</new_valor>" +
                    "<new_acrescimos>" + acrescimos + "</new_acrescimos>" +
                    "<new_condominio>" + condominio + "</new_condominio>" +
                    "<new_iptu>" + iptu + "</new_iptu>" +
                    "<new_energiaagua>" + energiaAgua + "</new_energiaagua>" +
                    "<new_totaldedespesas>" + totalDeDespesas + "</new_totaldedespesas>" +
        ...
        Xrm.Page.getAttribute("new_produtoinserido").setValue(true);
        alert("Dados Copiados do Cotrato Originador com Sucesso.");
        Xrm.Page.data.entity.save("save");
    }
}

function ComplementoDeContrato() {
    if (Xrm.Page.ui.getFormType() == 1 && Xrm.Page.getAttribute("new_tipodecontrato").getValue() == 2 && Xrm.Page.getAttribute("originatingcontract").getValue() !== null) {
        var entityRetrive = getEntityNodes("contract", "contractid", Xrm.Page.getAttribute("originatingcontract").getValue()[0].id);
        if (entityRetrive.length != 0) {
            var entityNodeRetrive = entityRetrive[0];
            var vencimento = entityNodeRetrive.selectSingleNode("q1:new_vencimentomescorrente");
            if (vencimento != null) { vencimento = vencimento.text; } else { vencimento = 0; }
            var cancelamento = entityNodeRetrive.selectSingleNode("q1:new_cancelamentodesconto");
            if (cancelamento != null) { cancelamento = cancelamento.text; } else { cancelamento = 0; }
            var multa = entityNodeRetrive.selectSingleNode("q1:new_multaescalonada");
            if (multa != null) { multa = multa.text; } else { multa = 0; }
            var aluguel = entityNodeRetrive.selectSingleNode("q1:new_aluguelescalonado");
            if (aluguel != null) { aluguel = aluguel.text; } else { aluguel = 0; }
            var vigencia = entityNodeRetrive.selectSingleNode("q1:new_prazodevigencia");
            if (vigencia != null) { vigencia = vigencia.text; } else { vigencia = 0; }
            var garantia = entityNodeRetrive.selectSingleNode("q1:new_tipodegarantia");
            if (garantia != null) { garantia = garantia.text; } else { garantia = 0; }
            var obsrevacao = entityNodeRetrive.selectSingleNode("q1:new_observacaodagarantia");
            if (obsrevacao != null) { obsrevacao = obsrevacao.text; } else { obsrevacao = ""; }
            var carencia = entityNodeRetrive.selectSingleNode("q1:new_carnciadias");
            if (carencia != null) { carencia = carencia.text; } else { carencia = 0; }
            Xrm.Page.getAttribute("new_vencimentomescorrente").setValue(vencimento);
            Xrm.Page.getAttribute("new_cancelamentodesconto").setValue(cancelamento);
            Xrm.Page.getAttribute("new_multaescalonada").setValue(multa);
            Xrm.Page.getAttribute("new_aluguelescalonado").setValue(aluguel);
            Xrm.Page.getAttribute("new_tipodegarantia").setValue(garantia);
            Xrm.Page.getAttribute("new_prazodevigencia").setValue(parseInt(vigencia));
            Xrm.Page.getAttribute("new_observacaodagarantia").setValue(obsrevacao);
            Xrm.Page.getAttribute('new_carnciadias').setValue(parseInt(carencia));
        }
    }
}